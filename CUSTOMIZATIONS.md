...
Tài liệu này tổng hợp toàn bộ các chỉnh sửa, tối ưu hóa và tính năng mới đã được thực hiện trên kho lưu trữ của bạn [Kde-caelestia](https://github.com/KhanhNguyen1603/Kde-caelestia.git) dựa trên phiên bản phát hành mới nhất **v2.2.2** (`ladybug-me/caelestia-dots-kde`).
...
## 4. Tùy chỉnh Giao diện Viền & Bo góc (Border & Rounding)

### 📐 Cấu hình mặc định Viền, Độ mịn và Bo góc (`borderconfig.hpp`)
* **Tệp tin ảnh hưởng:**
...
* **Chi tiết:** 
  * Đặt lại các thông số viền mặc định cho giao diện Shell Caelestia:
    * **Độ dày viền (`thickness`)**: `2` (viền mảnh 2px gọn đẹp).
    * **Bo góc (`rounding`)**: `10` (bo góc 10px).
    * **Độ mịn viền (`smoothing`)**: `20` (độ mịn viền 20px).
  * **Cách tự điều chỉnh ở máy người dùng (Runtime):** Mày có thể sửa hoặc thêm trực tiếp vào tệp cấu hình cá nhân tại `~/.config/caelestia/config.json`:
    ```json
    {
      "border": {
        "thickness": 2,
